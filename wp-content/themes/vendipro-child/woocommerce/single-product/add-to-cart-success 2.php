...
<div class="vp-modal prettyphoto-modal-load" id="vp-modal">
	<div class="vp-add-to-cart-success">
		<h3><?php _e( 'Thank you!', 'vendipro' ); ?></h3>
		...
		<p class="button-wrap">
			...
			<a class="button button-go-to-cart" href="<?php echo $woocommerce->cart->get_cart_url() ?>"><?php _e( 'Go to Cart', 'vendipro' ); ?></a>
		</p>

		<?php if ( function_exists( 'woocommerce_upsell_display' ) ) : ?>
			<?php woocommerce_upsell_display(); ?>
		<?php else : ?>
			<?php woocommerce_get_template( 'single-product/up-sells.php', array( 'posts_per_page' => 3, 'orderby' => apply_filters( 'woocommerce_upsells_orderby', $orderby ), 'columns' => 1 ) ); ?>
		<?php endif; ?>
