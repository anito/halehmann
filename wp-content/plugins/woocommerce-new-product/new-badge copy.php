...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	...
	function plugin_init() {
		load_plugin_textdomain( 'wbp-new-badge', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
	}
	...
	function init_new_products( $q ) {
		...
		if ( may_be_filtered_post() ) {
			...
			add_action( 'woocommerce_before_shop_loop_item', array( \Webpremiere\NewProducts, 'show_product_loop_new_flash' ), 30 );
			add_action( 'woocommerce_before_single_product_summary', array( \Webpremiere\NewProducts, 'show_single_product_new_flash' ), 30 );
	...
	// add_action( 'pre_get_posts', array( $this, 'init_new_products' ), 10, 1 );
	// add_action( 'pre_get_posts', 'init_new_products', 10, 1 );

	/**
	 * New Badge class
	 **/
	// namespace Webpremiere\NewProducts;
		...
		class WC_nb {
			...
			public function __construct() {
                if( !is_admin() ) {
					...
					if ( is_options_auto() ) {
						...
						add_action( 'woocommerce_before_shop_loop_item', array( $this, 'show_product_loop_new_flash' ), 30 );
						add_filter( 'woocommerce_before_single_product_summary', array( $this, 'show_single_product_new_flash' ), 30 );
					}
			...
			function init_settings() {
				...
				$this->settings = array(
					...
					array(
						'title' => __( 'Ermittlung nach Höchstalter', 'wbp-new-badge' ),
						'desc' => __( "Neue Produkte nach Höchstalter ermitteln.", 'wbp-new-badge' ),
						'id' => 'wc_nb_auto',
						'type' => 'checkbox',
						'default' => 'false',
						'checkboxgroup'   => 'start',
					...
					array(
						'title' => __( 'Höchstalter (in Tagen)', 'wbp-new-badge' ),
						'desc' => __( 'Anzahl der Tage ab Veröffentlichung, die das Produkt als <strong>neu</strong> gelten soll', 'wbp-new-badge' ),
						'desc_tip' => true,
						'id' => 'wc_nb_newness',
						'type' => 'number',
						'show_if_checked' => 'yes',
					...
					array(
						'title' => __( 'Label (Standard: ' . WC_nb::$default_label . ')', 'wbp-new-badge' ),
			...
			function admin_sections( $sections ) {
				$sections[ 'new_products'] = __( 'Neue Produkte', 'wbp-new-badge' );
				return  $sections;
			}
			...
			function output($classes) {
				...
				echo '<span class="wc-new-badge badge ' . $classes . '">' . __( $label, 'wbp-new-badge' ) . '</span>';
			...
			function  show_product_loop_new_flash() {
				$text = get_wc_nb_label();
				require_once( __DIR__ . '/templates/woocommerce/loop/new-flash.php' );
			}

			function  show_single_product_new_flash() {
				$text = get_wc_nb_label();
				require_once( __DIR__ . '/templates/woocommerce/single-product/new-flash.php' );
			}
