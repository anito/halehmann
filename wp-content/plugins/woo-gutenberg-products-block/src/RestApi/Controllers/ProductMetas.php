 ...
 */
class ProductMetas extends WC_REST_Products_Controller {
	 ...
	 * @var string
	 */
	protected $rest_base = 'product-meta';
	...
	public function get_item( $request ) {
		...
		foreach ( $ids as $id ) {
			...
			if ( ! wc_rest_check_post_permissions( $this->post_type, 'read', $product->get_id() ) ) {
				continue;
			}
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( ! $taxonomy || ! \taxonomy_exists( $taxonomy ) ) {
			return new \WP_Error( 'woocommerce_rest_taxonomy_invalid', __( 'Resource does not exist.', 'woo-gutenberg-products-block' ), array( 'status' => 404 ) );
		}
	...
	public function prepare_item_for_response( $item, $request ) {
		$taxonomy = \wc_attribute_taxonomy_name( $item->name );
		$data     = array(
